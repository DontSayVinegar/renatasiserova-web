...
> ⚠️ **Pozor na past:** Cloudflare v „Create" nabízí hlavně **Workers** — to NENÍ
> ono a skončí chybou „Missing entry-point to Worker script". Potřebuješ **Pages**.
> Pokud jsi omylem vytvořil Workers projekt, smaž ho (projekt → Settings → Delete)
> a začni znovu přes odkaz níže.

1. Otevři přímý odkaz na Pages flow (přeskočí matoucí obrazovku):
   **https://dash.cloudflare.com/?to=/:account/pages/new/provider/github**
   (ručně: Workers & Pages → Create → záložka/odkaz **Pages** → Connect to Git).
2. Povol přístup ke GitHubu a vyber repo `renatasiserova-web`.
3. Nastavení buildu: Framework preset **Astro** (build command `npm run build`,
   output `dist` — předvyplní se samo; když preset nevidíš, jsi ve Workers flow!).
4. **Save and Deploy.** Za ~2 minuty web běží na `něco.pages.dev` — zkontroluj ho!
5. Po kontrole: v projektu → **Custom domains** → **Add** → `renatasiserova.cz`
